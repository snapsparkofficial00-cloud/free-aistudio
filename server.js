    ...
    try {
        ...
        for (let i = 0; i < frameCount; i++) {
            try {
                ...
                const url = `https://image.pollinations.ai/prompt/${enhancedPrompt}?seed=${seed}&width=1280&height=720&nologo=true`;
                ...
                if (i < frameCount - 1) await new Promise(r => setTimeout(r, 2000));
            } catch (error) {
                console.error(`[AI Video] Frame ${i + 1} failed:`, error.message);
                // Create placeholder
                try {
                    ...
                    try {
                        ...
                        await execPromise(`ffmpeg -y -i "${svgPath}" -vf "scale=1280:720" "${pngPath}" 2>/dev/null`);
        ...
        if (images.length === 0) {
            return res.status(500).json({ success: false, error: 'Failed to generate any frames. Image API may be down.' });
        }

...
async function generateTrendingTopicsViaAI(region, category) {
    // Always return trending topics even without API
    const fallbackTopics = [
        { title: `Top 10 ${category} Trends in ${region} Right Now`, reason: 'High search volume', audience: 'General', potential: '500K+ views', hashtags: ['#trending', '#viral', `#${category}`] },
        { title: `Why ${category} is Changing Everything in 2026`, reason: 'Timely and relevant', audience: 'Tech enthusiasts', potential: '1M+ views', hashtags: ['#2026', '#future', '#tech'] },
        { title: `The ${category} Secret Nobody Talks About`, reason: 'Curiosity gap', audience: 'General', potential: '750K+ views', hashtags: ['#secret', '#exposed', '#viral'] },
        { title: `I Tested ${category} For 30 Days (Results)`, reason: 'Personal experience', audience: 'Lifestyle', potential: '600K+ views', hashtags: ['#challenge', '#results', '#test'] },
        { title: `${category} vs Reality: The Truth`, reason: 'Controversial angle', audience: 'General', potential: '900K+ views', hashtags: ['#truth', '#reality', '#vs'] }
    ];

    if (!OPENROUTER_API_KEY) return fallbackTopics;
...
async function generateViralSEO(topic, platform) {
    ...
    const powerWords = ['SHOCKING', 'INSANE', 'UNBELIEVABLE', 'SECRET', 'MIND-BLOWING', 'EXPOSED', 'REVEALED'];
...
In this ${platform} video, we break down everything about ${topic}.
...
#viral #trending #${topic.replace(/\s+/g, '')}`,
        hashtags: ['#viral', '#trending', '#youtube', '#shorts', '#fyp', '#foryou', '#viralvideo', '#mustwatch', '#insane', '#shocking', '#mindblown', '#hack', '#secret', `#${topic.replace(/\s+/g, '')}`, '#subscribe'],
        tags: [topic, 'viral', 'trending', 'youtube', 'shorts', 'fyp', 'foryou', 'hack', 'secret', 'tips'],
        bestTime: '3:00 PM - 5:00 PM EST (Peak Engagement)',
...
Provide ONLY a valid JSON object with these exact keys:
- titles: array of 5 attention-grabbing titles
- description: string with timestamps, CTAs
- hashtags: array of 15 hashtags
- tags: array of 10 SEO keywords
- bestTime: string with timezone
- viralScore: number 70-100
- thumbnailTexts: array of 3 text options`;
            ...
            if (jsonMatch) {
                ...
                // Merge AI data with base, keeping base as fallback
                return {
                    titles: aiData.titles || baseResult.titles,
                    description: aiData.description || baseResult.description,
                    hashtags: aiData.hashtags || baseResult.hashtags,
                    tags: aiData.tags || baseResult.tags,
                    bestTime: aiData.bestTime || baseResult.bestTime,
                    viralScore: aiData.viralScore || baseResult.viralScore,
                    thumbnailTexts: aiData.thumbnailTexts || baseResult.thumbnailTexts
                };
            }
        } catch (e) {
            console.log('[ViralSEO] AI enhancement failed, using base content:', e.message);
        }
    }
...
app.post('/api/assemble-video', async (req, res) => {
    ...
    try {
        const { script, topic, imageCount = 30, voice = 'hi-IN-MadhurNeural', resolution = '1280x720', style = 'cinematic', aspectRatio = '16:9' } = req.body;

        // Determine resolution from aspect ratio if not explicitly provided
        let finalResolution = resolution;
        if (aspectRatio === '9:16') finalResolution = '720x1280';
        else if (aspectRatio === '1:1') finalResolution = '720x720';
        else if (aspectRatio === '16:9') finalResolution = '1280x720';
        ...
        // STEP 1: Generate frames with retry
        console.log('[Pipeline] Step 1/4: Generating frames...');
        let frameResult;
        let frameAttempts = 0;
        const maxFrameAttempts = 2;

        while (frameAttempts < maxFrameAttempts) {
            frameAttempts++;
            try {
                frameResult = await generateVideoFrames(script || topic, { 
                    frameCount: imageCount, 
                    outputDir: uploadsDir, 
                    style 
                });
                if (frameResult.success && frameResult.frames.length > 0) break;
                if (frameAttempts < maxFrameAttempts) {
                    console.log(`[Pipeline] Frame gen attempt ${frameAttempts} failed, retrying...`);
                    await new Promise(r => setTimeout(r, 3000));
                }
            } catch (e) {
                console.log(`[Pipeline] Frame gen error (attempt ${frameAttempts}): ${e.message}`);
                if (frameAttempts >= maxFrameAttempts) throw e;
            }
        }

        if (!frameResult || !frameResult.success || frameResult.frames.length === 0) {
            throw new Error('FRAME_GENERATION_FAILED: No frames generated after retries');
        }
        ...
        const validFrames = frameResult.frames.filter(f => {
            ...
            if (!fs.existsSync(filepath)) {
                console.log(`[Pipeline] Frame ${f.index} missing: ${filepath}`);
                return false;
            }
            try {
                const buf = fs.readFileSync(filepath);
                const magic = buf.toString('hex', 0, 4).toLowerCase();
                const isImg = magic.startsWith('8950') || magic.startsWith('ffd8') || magic.startsWith('5249') || magic.startsWith('4749');
                if (!isImg) console.log(`[Pipeline] Frame ${f.index} invalid format: ${magic}`);
                return isImg;
            } catch(e) { return false; }
        });

        if (validFrames.length === 0) throw new Error('FRAME_VALIDATION_FAILED: All frames invalid');
        if (validFrames.length < frameResult.frames.length) {
            console.log(`[Pipeline] Using ${validFrames.length}/${frameResult.frames.length} valid frames`);
        }
        ...
        const videoResult = await assembleVideo(validFrames, voiceResult.audioPath, { resolution: finalResolution, outputDir: uploadsDir });

        // STEP 4: Save
        console.log('[Pipeline] Step 4/4: Saving...');
        ...
        res.status(500).json({
            ...
            analysis: { pattern: analysis.findings[0]?.pattern, description: analysis.findings[0]?.description, suggestedFix: analysis.findings[0]?.fix },
...
function getErrorSuggestions(analysis) {
    ...
    switch(fix) {
        ...
        case 'regenerate_frames':
            ...
            suggestions.push('Try generating with fewer frames (15 instead of 30).');
        ...
        case 'regenerate_audio':
            suggestions.push('Audio generation failed. Run: pip install edge-tts');
            break;
        default:
            suggestions.push('An unexpected error occurred. Try again with a different topic or fewer frames.');
    }
    return suggestions;
}
...
app.post('/api/zingroll/generate', async (req, res) => {
    try {
        const { topic, platform = 'shorts', duration = 30, style = 'cinematic', voice = 'hi-IN-MadhurNeural', language = 'english' } = req.body;
        if (!topic) return res.status(400).json({ success: false, error: 'Topic is required', code: 'MISSING_TOPIC' });

        console.log(`[ZingRoll API] Generating ${platform} video: "${topic}"`);
        const result = await generateZingRoll(topic, { platform, duration: Math.min(duration, 60), style, voice, outputDir: uploadsDir, language, generateViralPackage: true });
        if (!result.success) throw new Error(result.error || 'ZingRoll generation failed');

        const videos = readDB('videos');
        videos.unshift({ id: Date.now(), type: 'zingroll', topic, platform, duration: result.video.duration, videoUrl: result.video.videoUrl, thumbnailUrl: result.files.thumbnail?.url, viralPackage: result.viralPackage, script: result.script, sizeMB: result.video.sizeMB, createdAt: new Date().toISOString() });
        writeDB('videos', videos.slice(0, 50));

        res.json({ success: true, message: `ZingRoll ${platform} video created!`, video: result.video, viralPackage: result.viralPackage, script: result.script, files: result.files, suggestions: { title: result.viralPackage?.title, description: result.viralPackage?.description, tags: result.viralPackage?.tags, hashtags: result.viralPackage?.hashtags, thumbnailText: result.viralPackage?.thumbnailText, bestTime: result.viralPackage?.bestPostingTime, cta: result.viralPackage?.cta, captions: result.viralPackage?.captions } });
    } catch (error) {
        console.error('[ZingRoll API] Error:', error.message);
        res.status(500).json({ success: false, error: error.message, code: 'ZINGROLL_ERROR', suggestion: 'Try with a different topic or shorter duration' });
    }
});
...
app.post('/api/zingroll/viral-package', async (req, res) => {
    try {
        ...
        res.json({ success: true, topic, platform, viralPackage });
...
app.get('/api/zingroll/trending', async (req, res) => {
    try {
        ...
        res.json({ success: true, topics: topics.slice(0, parseInt(limit)), region, category, updatedAt: new Date().toISOString() });
    } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.post('/api/zingroll/quick', async (req, res) => {
    try {
        ...
        if (!topics[trendingIndex]) return res.status(400).json({ success: false, error: 'Invalid trending index' });
        const topic = topics[trendingIndex].topic;
        const result = await generateZingRoll(topic, { platform, outputDir: uploadsDir });
        res.json({ success: result.success, topic, video: result.video, viralPackage: result.viralPackage, trendingScore: topics[trendingIndex].score });
    } catch (error) { res.status(500).json({ success: false, error: error.message }); }
});
...
app.get('/api/agents/error-agent/status', (req, res) => {
    const agent = new ErrorFixingAgent();
    res.json({ success: true, stats: agent.getStats(), recentFixes: agent.getFixHistory(20) });
});

app.post('/api/agents/error-agent/analyze', (req, res) => {
    const { errorMessage, context } = req.body;
    if (!errorMessage) return res.status(400).json({ success: false, error: 'errorMessage required' });
    const agent = new ErrorFixingAgent();
    const analysis = agent.analyzeError(errorMessage, context);
    res.json({ success: true, analysis, suggestions: getErrorSuggestions(analysis) });
});

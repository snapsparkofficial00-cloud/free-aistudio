// ============================================================
// NEUROTUBE AI - FULLY WORKING BACKEND SYSTEM v2.0
// Real API Integrations: OpenRouter, ElevenLabs, YouTube Data API, HIGGSFIELD AI
...
// ============================================================
const higgsfieldRoutes = require('./higgsfield-routes');
const {
    HiggsfieldClient,
    generateBRoll,
    generateThumbnail,
...
const CONFIG = {
    ...
    higgsfield: {
        ...
        apiUrl: process.env.HIGGSFIELD_API_URL || 'https://api.higgsfield.ai',
    },
    system: {
...
const DB_PATHS = {
    ...
    higgsfield: path.join(__dirname, 'data', 'higgsfield_db.json'),
};
...
const DB = {
    ...
    system: loadDB(DB_PATHS.system, {
        uptime: 0,
        apiCalls: { openrouter: 0, elevenlabs: 0, youtube: 0, higgsfield: 0 },
        errors: [],
        improvements: [],
        version: '2.0.0'
    }),
    higgsfield: loadDB(DB_PATHS.higgsfield, {
        generations: [],
        videos: [],
        images: [],
        analyses: [],
...
// Make DB available globally for Higgsfield routes
global.DB = DB;
global.DB_PATHS = DB_PATHS;
...
async function getElevenLabsVoices() {
    ...
    try {
        const response = await axios.get(
            ...
            { headers: { 'xi-api-key': CONFIG.elevenlabs.apiKey }, timeout: 10000 }
        );
    ...
    try {
        ...
        if (req.body.thumbnailPath) {
            await youtubeClient.thumbnails.set({
                ...
                media: { body: fs.createReadStream(req.body.thumbnailPath) }
            });
        }
...
app.get('/api/youtube/analytics', async (req, res) => {
    try {
        if (!youtubeClient) return res.status(500).json({ error: 'YouTube API not configured' });
...
Make it engaging, authentic, and optimized for retention. The first 30 seconds must be impossible to skip.`;
        ...
        ], { model: CONFIG.openrouter.models.script, temperature: 0.85, max_tokens: 4000, trackPrompt: 'script' });
...
Generate viral titles and thumbnail concepts.`;
        ...
        const result = await callOpenRouter([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt }
        ], { model: CONFIG.openrouter.models.creative, temperature: 0.9, max_tokens: 2000, trackPrompt: 'thumbnail' });
        ...
        res.json({ success: true, analysis: result.content, topic, style, model: result.model });
...
app.post('/api/agents/voice', async (req, res) => {
    try {
        ...
        const result = await generateVoice(text, { voiceId, language, stability, similarity_boost, style });
        res.json(result);
...
Provide actionable trend intelligence.`;
        ...
        const result = await callOpenRouter([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt }
        ], { model: CONFIG.openrouter.models.analysis, temperature: 0.8, max_tokens: 3000, trackPrompt: 'trends' });
        ...
        res.json({ success: true, trends, raw: result.content, model: result.model });
...
Provide strategic intelligence I can act on today.`;
        ...
        const result = await callOpenRouter([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt }
        ], { model: CONFIG.openrouter.models.analysis, temperature: 0.75, max_tokens: 3000, trackPrompt: 'competitor' });
        ...
        res.json({ success: true, analysis: result.content, competitors: competitorChannels, model: result.model });
...
Provide complete optimization package.`;
        ...
        const result = await callOpenRouter([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt }
        ], { model: CONFIG.openrouter.models.creative, temperature: 0.7, max_tokens: 2500, trackPrompt: 'growth' });

        res.json({ success: true, optimization: result.content, model: result.model });
...
Provide 3 music generation prompts and 5 sound effect descriptions.`;
        ...
        const result = await callOpenRouter([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: userPrompt }
        ], { model: CONFIG.openrouter.models.creative, temperature: 0.9, max_tokens: 1500, trackPrompt: 'music' });
...
// 9. CinemaForge AI — Higgsfield Video Generation Agent
app.post('/api/agents/cinemaforge', async (req, res) => {
    try {
        const {
            prompt,
            model = 'veo3_1',
        ...
        if (!hf.isAuthenticated) {
            return res.status(503).json({
                success: false,
                error: 'Higgsfield not authenticated. Install CLI: npm install -g @higgsfield/cli && higgsfield auth login',
                setup: true,
            });
        }

        console.log('🎬 CinemaForge: Generating AI video...');
        const result = await hf.generateVideo(prompt, {
            model, duration, resolution, aspectRatio, mode, sound, genre, wait: true,
        });

        const videoUrl = result.result_url || result.url;

        // Track in Higgsfield DB
        DB.higgsfield.videos.push({
            id: Date.now(),
            type: 'text-to-video',
            prompt,
            model,
        ...
        saveDB(DB_PATHS.higgsfield, DB.higgsfield);

        DB.system.apiCalls.higgsfield++;
        saveDB(DB_PATHS.system, DB.system);

        res.json({
            success: true,
            agent: 'CinemaForge AI',
            videoUrl,
            jobId: result.job_id,
            model,
            duration,
        ...
        console.error('❌ CinemaForge error:', error);
        res.status(500).json({ error: error.message });
    }
});

// 10. VisualSynth AI — Higgsfield Image Generation Agent
app.post('/api/agents/visualsynth', async (req, res) => {
    try {
        const {
            prompt,
            model = 'nano_banana_2',
        ...
        console.log('🎨 VisualSynth: Generating AI image...');
        ...
        console.error('❌ VisualSynth error:', error);
        res.status(500).json({ error: error.message });
    }
});

// 11. ViralPredictor AI — Higgsfield Brain Activity Agent
app.post('/api/agents/viralpredictor', upload.single('video'), async (req, res) => {
    try {
        ...
        console.log('🧠 ViralPredictor: Analyzing video...');
        ...
        const suggestions = [];
        if (analysis.hookStrength < 7) suggestions.push('First 3 seconds need stronger hook');
        if (analysis.retentionPrediction < 6) suggestions.push('Add pattern interrupts every 30 seconds');
        if (analysis.viralPotential < 7) suggestions.push('Add emotional peak or shareable moment');
        if (analysis.overallScore >= 8) suggestions.push('Excellent viral potential! Post during peak hours.');

        DB.higgsfield.analyses.push({
            id: Date.now(),
...
// 12. ShortsForge AI — Higgsfield Shorts Creation Agent
app.post('/api/agents/shortsforge', upload.single('video'), async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ error: 'Video file required' });

        const { targetDuration = 60, highlightTimestamp } = req.body;
        ...
        console.log('📱 ShortsForge: Creating YouTube Shorts...');

        // Step 1: Reframe to 9:16
        const reframed = await hf.reframeVideo(req.file.path, '9:16', '720p', { wait: true });
        const shortsUrl = reframed.result_url || reframed.url;

        // Step 2: Analyze virality
        const shortsPath = path.join(uploadsDir, `shorts_${Date.now()}.mp4`);
        // Download shorts for local analysis
        ...
        const writer = fs.createWriteStream(shortsPath);
        ...
        const virality = await hf.analyzeVirality(shortsPath, { wait: true });
        ...
        DB.higgsfield.videos.push({
            ...
            shortsUrl,
            virality: {
                score: virality.overall_score || virality.viral_score || 0,
            },
            timestamp: new Date().toISOString(),
        });
        saveDB(DB_PATHS.higgsfield, DB.higgsfield);

        DB.system.apiCalls.higgsfield += 2;
        saveDB(DB_PATHS.system, DB.system);

        res.json({
            success: true,
            agent: 'ShortsForge AI',
            shortsUrl,
            viralityScore: virality.overall_score || virality.viral_score || 0,
            duration: targetDuration,
            sourceVideo: req.file.filename,
        });

    } catch (error) {
        console.error('❌ ShortsForge error:', error);
        res.status(500).json({ error: error.message });
    }
});

// ============================================================
// FULL VIDEO PIPELINE (v2.0 with Higgsfield)
// ============================================================

app.post('/api/pipeline/generate', async (req, res) => {
    try {
        const { topic, language, niche, videoType, quality, style, tone, generateVideo = false } = req.body;

        console.log('🎬 Starting Full Video Pipeline v2.0...');
        console.log(`   Topic: ${topic}`);
        console.log(`   Language: ${language}`);
        console.log(`   Style: ${style}`);
        console.log(`   Generate AI Video: ${generateVideo}`);
        ...
        console.log('   Step 2/6: Generating Voice...');
        ...
        // Step 3: Generate Thumbnail Concepts (OpenRouter)
        console.log('   Step 3/6: Generating Thumbnail Concepts...');
        ...
        console.log('   Step 4/6: Analyzing Trends...');
        ...
        console.log('   Step 5/6: SEO Optimization...');
        ...
        // Step 6: Higgsfield AI Video Generation (NEW!)
        if (generateVideo) {
            console.log('   Step 6/6: Generating AI Video with Higgsfield...');
            try {
                const hf = new HiggsfieldClient();
                if (hf.isAuthenticated) {
                    // Create a video prompt from the script
                    const videoPrompt = `YouTube video about ${topic}, ${style} style, professional cinematography, smooth camera movement, high quality footage`;

                    const videoRes = await axios.post(`http://localhost:${PORT}/api/agents/cinemaforge`, {
                        prompt: videoPrompt,
                        model: videoType === 'short' ? 'kling3_0' : 'veo3_1',
                        duration: videoType === 'short' ? 5 : 10,
                        resolution: quality === '4k' ? '1080p' : '1080p',
                        aspectRatio: videoType === 'short' ? '9:16' : '16:9',
                        mode: 'pro',
                        sound: 'off',
                    });
                    pipelineResults.steps.aiVideo = videoRes.data;
                } else {
                    pipelineResults.steps.aiVideo = {
                        error: 'Higgsfield not authenticated',
                        setup: 'Install CLI: npm install -g @higgsfield/cli && higgsfield auth login'
                    };
                }
            } catch (e) {
                pipelineResults.steps.aiVideo = { error: e.message };
            }
        } else {
            pipelineResults.steps.aiVideo = { skipped: true, reason: 'generateVideo=false' };
        }
        ...
        res.json({
            ...
            status: 'complete',
            higgsfieldEnabled: generateVideo,
            nextSteps: generateVideo
                ? [
                    '1. Review and edit the generated script',
                    '2. Generate full voiceover using the complete script',
                    '3. AI Video has been generated — download and review',
                    '4. Use thumbnail concepts to create final thumbnail',
                    '5. Edit and combine AI video + voiceover',
                    '6. Upload via /api/youtube/upload'
                ]
                : [
                    '1. Review and edit the generated script',
                    '2. Generate full voiceover using the complete script',
                    '3. Use thumbnail concepts to create actual thumbnails',
...
app.post('/api/system/self-improve', async (req, res) => {
    try {
        ...
        const higgsfieldGens = db.higgsfield.videos.slice(-10);
...
- Higgsfield AI videos generated: ${db.higgsfield.videos.length}
- Higgsfield images generated: ${db.higgsfield.images.length}
...
3. HIGGSFIELD OPTIMIZATION - Best models and settings for video generation
4. SYSTEM OPTIMIZATIONS - Technical improvements to the pipeline
5. NEW CAPABILITIES - What new features should be added
6. ERROR PREVENTION - How to avoid recent failures
...
Return actionable improvements as a structured plan.`;
        ...
        ], { model: CONFIG.openrouter.models.analysis, temperature: 0.9, max_tokens: 3000 });
        ...
        db.learning.optimization_rules.push({
            ...
            metrics: {
                ...
                higgsfieldVideos: db.higgsfield.videos.length,
...
app.get('/api/system/status', (req, res) => {
    ...
    const status = {
        ...
        databases: {
            ...
            higgsfield: {
                videos: DB.higgsfield.videos.length,
                images: DB.higgsfield.images.length,
                analyses: DB.higgsfield.analyses.length,
            }
        },
        apiCalls: DB.system.apiCalls,
        ...
        higgsfield: {
            ...
            mode: CONFIG.higgsfield.mode,
            models: {
                video: Object.keys(require('./higgsfield').HIGGSFIELD_CONFIG.models.video).length,
                image: Object.keys(require('./higgsfield').HIGGSFIELD_CONFIG.models.image).length,
            }
        }
    };
...
app.get('/api/system/health', (req, res) => {
    ...
    res.json({
        ...
        services: {
            ...
            higgsfield: hf.isAuthenticated ? 'connected' : 'disconnected',
        }
    });
});
...
app.get('/api/analytics/dashboard', (req, res) => {
    ...
    const hfVideos = DB.higgsfield.videos;
    const hfImages = DB.higgsfield.images;
    ...
    res.json({
        ...
        stats: {
            ...
            higgsfieldVideos: hfVideos.length,
            higgsfieldImages: hfImages.length,
            ...
            higgsfieldActivity: hfVideos.slice(-5).map(v => ({
                type: v.type,
                model: v.model,
                date: v.timestamp,
            })),
...
app.listen(PORT, () => {
    ...
    console.log('\n');
    console.log('╔══════════════════════════════════════════════════════════════╗');
    console.log('║           🧠 NEUROTUBE AI v2.0 — FULLY OPERATIONAL          ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');
    ...
    console.log(`║    🎬 Higgsfield:  ${hf.isAuthenticated ? '✅ CONNECTED' : '❌ NEEDS AUTH'}          ║`);
    console.log('╠══════════════════════════════════════════════════════════════╣');
    console.log('║  AI Agents: 12 Active                                        ║');
    console.log('║    ScriptMaster | VisualForge | VoiceSynth | AudioWeaver    ║');
    console.log('║    CinemaAI 4K | TrendOracle | CompetitorSpy | GrowthEngine ║');
    console.log('║    🆕 CinemaForge | 🆕 VisualSynth | 🆕 ViralPredictor      ║');
    console.log('║    🆕 ShortsForge                                            ║');
    console.log('╠══════════════════════════════════════════════════════════════╣');

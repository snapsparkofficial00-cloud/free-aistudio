 ...
 * Uses Hugging Face Inference API + SVG Placeholder Fallback
 */
...
async function generateViralPackageHermes(topic, platform = 'shorts', language = 'english') {
    ...
    if (!model) return generateRuleBasedViralPackage(topic, platform, language);
...
async function generateVerticalFrames(topic, script, duration, outputDir, style) {
    ...
    for (let i = 0; i < frameCount; i++) {
        ...
        for (const modelId of HF_MODELS) {
            if (success) break;
            try {
                console.log(`[ZingRoll] Frame ${i + 1}/${frameCount} via HF ${modelId}...`);
                const response = await axios.post(
                    `${HF_API_URL}/${modelId}`,
                    { inputs: `${topic}, ${style} style, vertical 9:16 format, mobile-optimized, professional, cinematic, scene ${i + 1}/${frameCount}` },
                    {
                ...
                if (buffer.length > 1000 && (buffer[0] === 0x89 || buffer[0] === 0xFF)) {
                    ...
                    const filename = `zingroll_frame_${Date.now()}_${i}.${ext}`;
                    const filepath = path.join(outputDir, filename);
                    fs.writeFileSync(filepath, buffer);
                    frames.push({ index: i, filepath, url: '/uploads/' + filename, duration: 2, source: 'huggingface' });
                    console.log(`[ZingRoll] Frame ${i + 1} saved (${(buffer.length/1024).toFixed(1)}KB)`);
                    success = true;
                }
            } catch (error) {
                console.log(`[ZingRoll] HF ${modelId} failed:`, error.message);
            }
        }
        ...
        if (!success) {
            try {
                const placeholder = await createZingRollPlaceholder(topic, outputDir, i);
                if (placeholder) {
                    frames.push(placeholder);
                    console.log(`[ZingRoll] Frame ${i + 1} placeholder created`);
                }
            } catch(e) {}
        }
...
async function createZingRollPlaceholder(topic, outputDir, index) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="720" height="1280" viewBox="0 0 720 1280">
        ...
        <rect x="5%" y="5%" width="90%" height="90%" fill="#1a1a2e" rx="40" stroke="#ff0040" stroke-width="3" opacity="0.8"/>
        <text x="50%" y="45%" font-family="Arial,sans-serif" font-size="36" font-weight="bold" fill="#ff0040" text-anchor="middle">ZINGROLL</text>
        <text x="50%" y="52%" font-family="Arial,sans-serif" font-size="20" fill="#e0e0e0" text-anchor="middle">${topic.substring(0, 30).replace(/&/g, '&amp;').replace(/</g, '&lt;')}</text>
        <text x="50%" y="58%" font-family="Arial,sans-serif" font-size="14" fill="#888" text-anchor="middle">Frame ${index + 1} | Placeholder</text>
        <text x="50%" y="70%" font-family="Arial,sans-serif" font-size="12" fill="#555" text-anchor="middle">Image API temporarily unavailable</text>
    </svg>`;

    const svgPath = path.join(outputDir, `temp_zr_${Date.now()}_${index}.svg`);
    const pngPath = path.join(outputDir, `zingroll_frame_${Date.now()}_${index}.png`);
    fs.writeFileSync(svgPath, svg);

    try {
        await execPromise(`ffmpeg -y -i "${svgPath}" -vf "scale=720:1280" "${pngPath}" 2>/dev/null`);
        fs.unlinkSync(svgPath);
    } catch(e) {
        fs.renameSync(svgPath, pngPath);
    }

    return { index, filepath: pngPath, url: '/uploads/' + path.basename(pngPath), duration: 2, source: 'svg-placeholder' };
}
        ...
        for (const modelId of HF_MODELS) {
            try {
                ...
                if (buffer.length > 1000) {
                    const ext = buffer[0] === 0x89 ? 'png' : 'jpg';
                    const filename = `zingroll_thumbnail_${Date.now()}.${ext}`;
                    const filepath = path.join(outputDir, filename);
                    fs.writeFileSync(filepath, buffer);

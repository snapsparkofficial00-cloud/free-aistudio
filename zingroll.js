...
async function generateVerticalFrames(topic, script, duration, outputDir, style) {
    ...
    for (let i = 0; i < frameCount; i++) {
        try {
            ...
            // Use working pollinations endpoint with longer timeout
            const url = `https://image.pollinations.ai/prompt/${prompt}?width=720&height=1280&seed=${seed}&nologo=true`;
            ...
            console.log(`[ZingRoll] Frame ${i + 1}/${frameCount} generated (${(buffer.length/1024).toFixed(1)}KB)`);
            ...
            try {
                ...
                if (placeholder) frames.push(placeholder);
            } catch(e) {}
        }
    }
    return frames;
...
async function createZingRollPlaceholder(topic, outputDir, index) {
    ...
    return { index, filepath: pngPath, url: '/uploads/' + path.basename(pngPath), duration: 2 };
}
...
async function generateThumbnailImage(prompt, outputDir) {
    try {
        ...
        const url = `https://image.pollinations.ai/prompt/${encodedPrompt}?width=720&height=1280&seed=${seed}&nologo=true`;
        const response = await axios.get(url, { 
            responseType: 'arraybuffer', 
            timeout: 120000,
        ...
        const buffer = Buffer.from(response.data);
        if (buffer.length < 1000) throw new Error('Invalid thumbnail data');

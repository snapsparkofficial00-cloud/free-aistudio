...
async function generateSpeech(text, options = {}) {
    const {
        ...
        outputPath,
        rate = '-10%',        // Slightly slower for natural feel
        volume = '+10%',      // Louder, more present
        pitch = '+5Hz'        // Slightly higher pitch for engagement
    } = options;
 ...
 * Adds pauses, fixes punctuation for better flow
 */
function preprocessForTTS(text) {
    return text
        ...
        .replace(/\b(um|uh|ah|oh)\b/gi, '')  // Remove filler words
        .trim();
}

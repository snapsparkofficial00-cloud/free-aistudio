...
async function generateWithPollinations(prompt, options) {
    ...
    for (let i = 0; i < count; i++) {
        try {
            ...
            // Use pollinations.ai (working domain) with longer timeout
            const url = `https://image.pollinations.ai/prompt/${enhancedPrompt}?width=${width}&height=${height}&seed=${seed}&nologo=true`;
            ...
            if (buffer.length < 1000) {
                console.log(`     Image ${i + 1} returned empty/invalid data, trying fallback...`);
            ...
            if (!isPNG && !isJPEG) {
                console.log(`     Image ${i + 1} has wrong format (magic: ${buffer.slice(0,4).toString('hex')}), trying fallback...`);
            ...
            console.error(`     Image ${i + 1} failed:`, error.message);
            // Try fallback: create colored placeholder
            try {
                const placeholder = await createPlaceholderImage(prompt, width, height, outputDir, i);
                if (placeholder) {
                    images.push(placeholder);
                    console.log(`     Image ${i + 1} placeholder created instead`);
                }
            } catch(e) {}
        }
    }
    ...
    if (images.length === 0) {
        throw new Error('All image generation attempts failed. Check internet connection.');
    }
...
async function createPlaceholderImage(prompt, width, height, outputDir, index) {
    try {
        const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
            ...
            <rect x="5%" y="5%" width="90%" height="90%" fill="#16213e" rx="30" stroke="#00f3ff" stroke-width="2"/>
            <text x="50%" y="40%" font-family="Arial,sans-serif" font-size="28" font-weight="bold" fill="#00f3ff" text-anchor="middle">NeuroTube AI</text>
            <text x="50%" y="52%" font-family="Arial,sans-serif" font-size="16" fill="#e0e0e0" text-anchor="middle">${prompt.substring(0, 35).replace(/&/g, '&amp;').replace(/</g, '&lt;')}...</text>
            <text x="50%" y="62%" font-family="Arial,sans-serif" font-size="12" fill="#888" text-anchor="middle">Image ${index + 1} | Generated Placeholder</text>
            <text x="50%" y="75%" font-family="Arial,sans-serif" font-size="10" fill="#555" text-anchor="middle">API temporarily unavailable</text>
        </svg>`;

        const svgPath = path.join(outputDir, `temp_${Date.now()}_${index}.svg`);
        const pngPath = path.join(outputDir, `image_${Date.now()}_${index}.png`);
        fs.writeFileSync(svgPath, svg);

        // Try to convert with ffmpeg, if not available just return SVG

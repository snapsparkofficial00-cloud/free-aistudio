    ...
    for (let i = 0; i < actualCount; i++) {
        try {
            ...
            if (imageData) {
                ...
                console.log(`   ⚠ No image data in response ${i + 1}`);
            }
...
async function generateVideoFrames(script, options = {}) {
    ...
    console.log(`🎬 Generating ${frameCount} video frames from script...`);

    for (let i = 0; i < segments.length; i++) {
        const segment = segments[i];
        const framePrompt = `Scene ${i + 1}/${frameCount}: ${segment}. ${style} style, cinematic composition, professional photography, high detail, 16:9 aspect ratio.`;
        ...
        try {
            ...
            if (result.images.length > 0) {
                frames.push({
                    ...
                    duration: 1 // 1 second per frame = 30fps feel
                });
            }
...
function parseScriptIntoSegments(script, frameCount) {
    ...
    const cleanScript = script
        .replace(/\[METADATA\][\s\S]*?\[SCRIPT\]/gi, '')
        .replace(/\[0?:\d{2}[-–]\d{2}\s+\w+\]/gi, '')
        .replace(/\[B-ROLL:.*?\]/gi, '')
        .replace(/\[MUSIC CUE\]/gi, '')
        .replace(/\[EMOTION:.*?\]/gi, '')
        .trim();
    ...
    // Distribute sentences across frames
    const segmentsPerFrame = Math.ceil(sentences.length / frameCount);
    const segments = [];
    ...
    // Fill remaining frames with summary if needed
...
async function generateThumbnail(title, options = {}) {
    ...
    const prompt = `YouTube thumbnail for video titled: "${title}". ${style}. High contrast, bold colors, eye-catching, professional design, 16:9 aspect ratio. No text in image.`;
